...
public class UserRepositoryImpl implements UserRepository {
	...
	public RegisterBean authenticate(LoginBean login) {
		...
		try {
			...
			stmt.setString(1, login.getEmailId());
	...
	public boolean validate(ForgetBean forget) {
		String sql = "select * from users where emailId=? and movie=?";
		Connection conn = null;

		try {
			conn = getConnection();
			PreparedStatement stmt = conn.prepareStatement(sql);
			stmt.setString(1, forget.getEmailId());
	...
	public boolean persist(RegisterBean register) {
		...
		try {
			...
			stmt.setString(2, register.getEmailId());
